...
<div class="m-portlet">
                                    ...
                                    <!--begin::Form-->
                                      ...
                                      {{ Form::model($user, array('route' => array('users.update', $user->id), 'method' => 'PUT','class' => 'm-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed')) }}
             ...
             <option value="{{ $role->id }}"  @if($user->role_id==$role->id) selected="selected" @endif > {{ Form::label($role->name, ucfirst($role->name)) }}</option>
              @endforeach
